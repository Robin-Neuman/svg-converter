    ...
    for await (const file of files) {
        ...
        if (!isDirectory) {
            fs.readFile(`${path}/${file.name}`, 'utf-8', function (err, data) {
                ...
                const regex1 = new RegExp(`xmlns:xlink`, 'i');
                const regex2 = new RegExp(`xml:space`, 'i');
                const regex3 = new RegExp(`xmlns:serif`, 'i');
                const regex4 = new RegExp('serif:id', 'i');
            ...
            for await (const fileDeep of filesDeep) {
                ...
                if (!isDirectory2) {
                    ...
                    for await (const fileDeeper of filesDeeper) {
                        const isDirectory2 = (await fs.promises.lstat(`${path}/${file.name}/${fileDeep.name}/${fileDeeper.name}`)).isDirectory();
                        if (!isDirectory2) {
                            fs.readFile(`${path}/${file.name}/${fileDeep.name}/${fileDeeper.name}`, 'utf-8', function (err, data) {
                                if (err) throw err;
                                ...
                                fs.writeFile(`${path}/${file.name}/${fileDeep.name}/${fileDeeper.name}`, svg, 'utf-8', function (err, data) {
                                    if (err) throw err;
                                    console.log(`Conversion of ${fileDeeper.name} successful!`);
                                })
                            })
                        } else {
                            console.log(`Can't read deeper than one folder ( sorry! :< )`)
                        }
                    }
                }
            }
